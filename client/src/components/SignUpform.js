	...
	async function register(e) {
		...
		axios({
			...
			.catch((err) => {
				...
				setError(true);
				if (
					err.response.data.error === "password invalide" ||
					err.response.data.error === "Le mail existe deja"
				) {
					setError({
		...
		<div>
			...
			<>
				{formSubmit ? (
					<>
						<SignInform />
						<h4 className="success_register">
							Enregistrement réussi, un email de vérification a été envoyé.
						</h4>
					</>
				) : (
