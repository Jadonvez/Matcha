...
const signIn = async (req, res) => {
    ...
    try {
        ...
        const results = await pool.query("SELECT * FROM users WHERE mail = $1", [mail]);
        //if (results.rows[0])
...
function validateRegistration(user) {
    const validLogin = typeof user.login == "string" && user.login.trim() != "" && user.login.trim().match(/^(?=[a-zA-Z0-9]{5,12}$)/);
...
const register = (req, res) => {
    const { login, mail, name, firstname, password, dob, gender, orientation } = req.body;
    var err = validateRegistration(req.body);
    const uid = uuidv4();
    if (err.mail && err.password && err.login) {
        pool.query("INSERT INTO users (uid, login, mail, name, firstname, password, dob, gender, orientation) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", [uid, login, mail, name, firstname, bcrypt.hashSync(password, 10), moment(dob , "YYYY MM DD").add(10, 'hours'), gender, orientation],
        async (error, results) => {
            if (error)
