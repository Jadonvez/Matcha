...
const uploadProfil = async (req, res, next) => {
  try {
    ...
    try {
        ...
        pool.query("SELECT * FROM users WHERE uid = $1", [req.body.uid], (error, results) =>{
        if (error)
            res.status(401).send("error dans ma requete sql")
        else if (!results.rows.length)
            res.status(401).send("User est inconnu dans la db")
        else if (results.rows.length)
            if (results.rows[0].ppicture)
                pool.query("UPDATE users SET ppicture = $1 WHERE uid = $2", [PpicturePath, req.body.uid], (error, results) => {
                if (error)
                    throw error;
